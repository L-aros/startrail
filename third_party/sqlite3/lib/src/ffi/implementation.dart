...
final class FfiDatabaseImplementation extends DatabaseImplementation
  ...
  @override
  void applySqlCipherRawKey(Uint8List key) {
    if (key.length != 32) {
      throw ArgumentError.value(key.length, 'key.length', 'must be 32');
    }

    final databaseName = Utf8Utils.allocateZeroTerminated('main');
    try {
      final result = withClearedNativeBytes(
        key,
        (keyPointer) => libsqlite3.sqlite3_key_v2(
          ffiDatabase.db,
          databaseName,
          keyPointer.cast(),
          key.length,
      ...
      if (result != SqlError.SQLITE_OK) {
        throw SqliteException(
          extendedResultCode: result,
          message: 'Could not apply SQLCipher raw key',
        );
      }
    } finally {
      databaseName.free();
