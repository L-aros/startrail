...
GitHub Actions 的 `Build` 工作流在 `main`、版本标签、PR 和手动触发时运行：Ubuntu job 执行分析、Flutter/infrastructure 测试和 Android release APK 构建；Windows job 构建完整 Windows release 运行包。两项构建物仅作为 14 天的 Actions artifact 上传；正式 GitHub Release 必须由人工核验后创建，不自动发布。
